...
from dataLoader import AcuityTransformLoader
...
# calling dataLoader for only Contrast
train_loaders , val_loaders = AcuityTransformLoader()

# Best model parameters
best_model_path = 'v2_onlyAcuityBlur_DevelopmentalCurriculum.pth'
...
for age_idx, age_group in enumerate(age_groups):
    ...
    for epoch in range(32):  # 32 epochs per age group
        ...
        train_loss /= len(train_loaders[age_idx].dataset)
...
plt.title("Train and Validation Accuracy with AcuityBlur Transfrom")
plt.xlabel("Epochs")
plt.ylabel("Accuracy")
plt.legend()
...
plt.title("Train and Validation Loss with AcuityBlur Transfrom")
plt.xlabel("Epochs")
plt.ylabel("Loss")
plt.legend()
...
plot_path = "OnlyAcuityBlur_DevelopmentalCurriculum_epochs_plot.png"
