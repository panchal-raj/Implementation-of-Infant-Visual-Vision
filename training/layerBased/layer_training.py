...
from src.dataloader import create_curriculum_dataloaders, create_no_curriculum_dataloader
...
def train_layerwise_curriculum(model, curriculum_dataloaders, val_dataloader, ages, stage_epochs, criterion, optimizer, scheduler, total_epochs, model_output_dir, loss_output_dir, model_name):
    ...
    for age in ages:
        stage_name = f"LayerWise_Age_{age}mo_{model_name}"
    ...
    # Save the model
    torch.save(model.state_dict(), os.path.join(model_output_dir, f"{model_name}_layerwise_final.pth"))

    # Save losses
    loss_file = f"{model_name}_layerwise_losses.json"
    ...
    final_stage_name = f"LayerWiseCurriculum_{model_name}"
...
def train_layerwise(batch_size, total_epochs, learning_rate, num_classes, model_output_dir, loss_output_dir, model_names, ages=AGES, user_epoch_map=None):
    ...
    curriculum_dataloaders = create_curriculum_dataloaders(train_data, ages, batch_size)
...
def main():
    ...
    model_output_dir = "output/models/layerwise/"
    loss_output_dir = "output/loss_logs/layerwise/"
